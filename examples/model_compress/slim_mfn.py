...
from nni.compression.torch import SlimPruner, ModelSpeedup
...
#    pruner.export_model(model_path='./slim/pruned_mask_mfn.pth', mask_path='./slim/mask_mfn.pth')
    """
    if args.parallel:
    ...
    new_model.to(device)
    new_model.load_state_dict(torch.load('./slim/pruned_mask_mfn.pth'))
...
#    torch_graph = build_module_graph(model.to('cpu'), data)
#    print(torch_graph.name_to_node)

#    model.to('cpu')
#    model.forward(data)
    ms = ModelSpeedup(model.to('cpu'), data, './slim/mask_mfn.pth', torch.device('cpu'))
    ms.speedup_model()
    torch.save(model, './slim/pruned_mfn.pth')
